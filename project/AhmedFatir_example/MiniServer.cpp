...
private:
	...
	std::vector<struct pollfd> polls; //-> vector of pollfd
public:
...
void Server::ClearClients(int fd){ //-> clear the clients
	for(size_t i = 0; i < polls.size(); i++){ //-> remove the client from the pollfd
		if (polls[i].fd == fd)
			{polls.erase(polls.begin() + i); break;}
	}
	...
	while (Server::Signal == false) {
		...
		for (size_t i = 0; i < polls.size(); i++) {
			if (polls[i].revents & POLLIN) {
				if (polls[i].fd == SerSocketFd)
					AcceptNewClient();
				else
					ReceiveNewData(polls[i].fd);
			}
		}
	}
	CloseFds();
...
void Server::AcceptNewClient() {
	...
	polls.push_back(NewPoll); //-> add the client socket to the pollfd
...
void Server::SerSocket() {
	...
	if(setsockopt(SerSocketFd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) == -1) //-> set the socket option (SO_REUSEADDR) to reuse the address
